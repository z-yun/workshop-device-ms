  ...
  <div class="app-container">
    ...
    <div class="app-container-box">
      ...
      <el-table :data="dataList" class="mbt-24" border v-loading="listLoading" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="入库单号" width="150" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            {{ scope.row.purchaseStockInId }}
          </template>
        </el-table-column>
        <el-table-column label="入库日期" sortable prop="stockDate" width="150"></el-table-column>
        <el-table-column label="收货人" width="120">
          <template slot-scope="{row}">
            <div v-if="row.receiver">{{row.receiver.realName}}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="入库金额" prop="stockAmount" width="100"></el-table-column>
        <el-table-column label="操作" min-width="180px" fixed="right">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="handleAdd(scope.row.purchaseStockInId)">
              {{ $t('btn.look') }}
            </el-button>
            <el-button type="text" size="small" @click="handleAdd(scope.row.purchaseStockInId)">
              {{ $t('btn.edit') }}
...
import { getPurchaseStockInList, delPurchaseStockIn } from '@/api/psi'
...
export default {
  ...
  methods:{
    ...
    getList(){
      ...
      getPurchaseStockInList(param).then(res => {
    ...
    handleBatchDel(id){
      ...
      // if(this.multipleSelection.length>0){
        ...
        .then(_ => {
          // let purchaseStockInIds = []
          // this.multipleSelection.forEach(item => {
          // 	purchaseStockInIds.push(item.purchaseStockInId)
          // })
          // let param = {purchaseStockInIds}
          let param = {
            purchaseStockInIds: [id]
          }
          delPurchaseStockIn(param).then(res => {
            this.listLoading = false
            if(res.code == '200'){

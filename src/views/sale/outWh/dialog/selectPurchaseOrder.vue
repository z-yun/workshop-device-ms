<template>
  <el-dialog :visible.sync="visible" :title="$t('psi.selectSaleOrder')" :before-close="handleClose" :close-on-click-modal="false" width="960px" append-to-body top="10vh">
    <div class="dialog-scroll-wrap">
      <el-form :inline="true" ref="searchForm" :model="searchForm" class="search-form app-container-box" :rules="rules">
        <el-form-item prop="supplierId">
          <el-select v-model="searchForm.supplierId" placeholder="购买客户">
            <el-option
        ...
        <el-form-item prop="purchaseOrderId">
          <el-input v-model="searchForm.purchaseOrderId" placeholder="请输入客户采购/销售订单号" />
        </el-form-item>
        <!-- <el-form-item prop="stockStatus">
          <el-select v-model="searchForm.stockStatus" placeholder="出库状态">
            <el-option
      ...
      <el-table :data="dataList" class="mbt-24" border v-loading="listLoading">
        ...
        <el-table-column label="销售订单号" width="150">
          <template slot-scope="scope">
            <span class="primary-txt">{{ scope.row.purchaseOrderId }}</span>
          </template>
        </el-table-column>
        <el-table-column label="发货人" width="180">
          <template slot-scope="{row}">
            <div v-if="row.purchaseUser">{{row.purchaseUser.realName}}</div>
          </template>
        </el-table-column>
        <el-table-column label="销售日期" sortable prop="purchaseDate" width="150"></el-table-column>
        <el-table-column label="发货日期" sortable prop="deliveryDate" width="150"></el-table-column>
        <el-table-column label="购买客户" width="150">
          <template slot-scope="{row}">
        ...
        <el-table-column label="销售金额" prop="totalAmount" width="100"></el-table-column> -->
       <el-table-column label="状态">
          <template slot-scope="{row}">
             <div v-if="row.orderStatus">{{row.orderStatus.desc}}</div>
          </template>
        </el-table-column>
        <!-- <el-table-column label="已收款" prop="paidAmount" width="100"></el-table-column> -->
      </el-table>
      <!-- component -->
...
import { getSaleOrderList } from '@/api/psi'
...
export default{
  ...
  data(){
    return {
     searchForm:{},
     rules:{
       purchaseOrderId:[{ required: true, trigger: 'blur', message:this.$t('validate.required')}],
     },
     page: {
       page: 1,
       limit: 10,
  ...
  methods:{
    ...
    getList(){
      if(this.searchForm.purchaseOrderId){
        ...
        getSaleOrderList(param).then(res => {

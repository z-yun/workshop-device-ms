    ...
    <el-form :model="addForm" :rules="rules" ref="addForm" label-width="110px" class="addForm search-form">
      ...
      <div class="app-container-box">
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="发货人" prop="receiverId">
      ...
      <div class="app-container-box">
        ...
        <el-table
          ...
          <el-table-column label="已出库数" prop="finishQuantity" width="120"></el-table-column>
          <el-table-column label="可出库数" prop="remainQuantity" width="120"></el-table-column>
          <el-table-column label="操作" fixed="right" min-width="120">
            <template slot-scope="{row}" v-if="source=='edit'">
              <el-button v-if="row.status==0" type="text" size="small" @click="handleStockIn(row,1)">
                {{ $t('psi.stockOut') }}
              </el-button>
              <!-- <el-button v-if="row.status==1" type="text" size="small" @click="handleStockIn(row,0)">
...
import { getWarehouseList, saveSaleStockOut, viewSaleStockOut, viewSaleOrder, saleStockOut } from '@/api/psi'
  ...
  methods:{
    ...
    handleSubmit(form){
      this.$refs[form].validate((valid, props) => {
          ...
          saveSaleStockOut({purchaseStockIn}).then(res => {
    ...
    handleStockIn(row,status){
      ...
      if(status == 0){
        ...
        if(row.quantity > row.remainQuantity){
          this.$message.info(this.$t('psi.stockOutQuatity'))
          return false
        }
      }
    ...
    queryPurchaseStockIn(){
      ...
      viewSaleStockOut(param).then(res => {
        if(res.code == '200'){

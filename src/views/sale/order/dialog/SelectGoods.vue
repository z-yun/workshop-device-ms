  ...
  <el-dialog :visible.sync="visible" :title="$t('psi.selectGoods')" :before-close="handleClose" :close-on-click-modal="false" width="960px" append-to-body top="10vh">
    <div class="dialog-scroll-wrap">
      ...
      <el-table :data="dataList" class="mbt-24" border v-loading="listLoading" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="商品名称" prop="name" width="140" :show-overflow-tooltip="true"></el-table-column>
        ...
        <el-table-column label="客户采购订单号" prop="purchaseOrderNo" width="140"></el-table-column>
        <el-table-column label="库存数量" prop="createTime"></el-table-column>
        <el-table-column label="供应商" :show-overflow-tooltip="true">
          <template slot-scope="{row}">
						<div v-if="row.supplier">{{row.supplier.name}}</div>
          </template>
        </el-table-column>
        <el-table-column label="来源" prop="createTime"></el-table-column>
  ...
  methods:{
    ...
    getList(){
      let param = {
        ...
        ...this.searchForm,
        excludeProductIds:this.excludeProductIds.concat(this.selectIds)
      }

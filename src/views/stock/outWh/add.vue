  ...
  <div class="app-container detail">
    <el-form :model="addForm" :rules="rules" ref="addForm" label-width="110px" class="addForm search-form">
      ...
      <div class="app-container-box">
        <h4 class="title">{{$t('psi.stockOutInfo')}}</h4>
        <el-row>
          <el-col :span="8">
            <el-form-item label="出库单号">
              <el-input v-if="source=='edit'" v-model="addForm.otherStockInId" disabled placeholder="请输入" />
              <el-input v-else disabled placeholder="请输入" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="发货人" prop="receiverUserId">
              <el-select v-model="addForm.receiverUserId" placeholder="请选择">
                <el-option
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="出库日期" prop="stockDate">
              <el-date-picker
      ...
      <div class="app-container-box">
        <h4 class="title">
          <span>{{$t('psi.stockOutDetail')}}</span>
        ...
        <el-table
          ...
          <el-table-column label="操作" fixed="right" min-width="160">
            <template slot-scope="scope">
              <!-- status 0或undefined未入库 1已入库 -->
              <el-button v-if="source=='edit'&&scope.row.status!=1" type="text" size="small" @click="handleStockIn(scope.row,1)">
                {{ $t('psi.stockOut') }}
              </el-button>
              <!-- <el-button v-if="source=='edit'&&scope.row.status==1" type="text" size="small" @click="handleStockIn(scope.row,0)">
                {{ $t('psi.cancelStockOut') }}
              </el-button> -->
              ...
              <!-- <el-button v-if="scope.row.status!=1" type="text" class="danger-txt" size="small" @click="handleRemove(scope)">
                {{ $t('btn.remove') }}
              </el-button> -->
            </template>
          </el-table-column>
...
import { getWarehouseList, saveOtherStockOut, viewOtherStockOut, otherStockOut } from '@/api/psi'
  ...
  data() {
    return {
      ...
      rules:{
        ...
        receiverUserId:[{ required: true, trigger: 'blur', message:this.$t('validate.required')}]
      },
      userList:[],
      warehouseList:[],
      stockStatusOptions:[
  ...
  methods:{
    ...
    handleSubmit(form){
      this.$refs[form].validate((valid, props) => {
          ...
          let products = this.products.map(item=>{
            if(this.source == 'edit'){
              return {
                ...
                otherStockInProductId:item.otherStockInProductId
              }
            } else {
              return {
                productId:item.id,
          ...
          if(this.addForm.deleted){
            delete this.addForm.deleted
          }
          ...
          let param = {
            products,
            otherStockIn:{...this.addForm}
          }

          saveOtherStockOut(param).then(res => {
    ...
    handleStockIn(row,status){
      // 出库、取消操作
      let param = {
        otherStockInId:this.id,
        productId:row.id,
        warehouseId:this.addForm.warehouseId,
        quantity:row.quantity,
        status:status
      }
      otherStockOut(param).then(res => {
        if(res.code == '200'){
          this.$message.success(this.$t('validate.success'))
          this.queryOtherStockIn()
        }
      })
    },
   ...
   queryOtherStockIn(){
     ...
     viewOtherStockOut(param).then(res => {
       if(res.code == '200'){
        this.addForm = JSON.parse(JSON.stringify(res.data.otherStockIn))
        ...
        for(let i=0;i<products.length;i++){
          ...
          obj.otherStockInProductId = products[i].otherStockInProductId
          obj.edit = false
          this.products.push(obj)
        }
       }

  ...
  <div class="app-container">
    ...
    <div class="app-container-box">
      ...
      <el-table :data="dataList" class="mbt-24" border v-loading="listLoading" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="控制器名称" width="140" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            {{ scope.row.name }}
          </template>
        </el-table-column>
        <el-table-column label="控制器IP" width="140">
          <template slot-scope="scope">
            {{ scope.row.ip }}
          </template>
        </el-table-column>
        <el-table-column label="(com3)三色灯">
          <template slot-scope="{row}">
            <div v-if="row.light">{{row.light.name}}</div>
          </template>
        </el-table-column>
        <!-- <el-table-column label="(com4)轨道">
          <template slot-scope="{row}">
            <div v-if="row.rail">{{row.rail.name}}</div>
          </template>
        </el-table-column> -->
        <el-table-column label="硬件编码" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            {{ scope.row.ioFunction }}
          </template>
        </el-table-column>
...
import { getIoList, delIo } from '@/api/factory'
    ...
    handleBatchDel(id){
      ...
      // if(this.multipleSelection.length>0){
        ...
        .then(_ => {
          ...
          // 	ioConfigIds.push(item.id)
          // })
          // let param = {ioConfigIds}
          let param = {
            ioConfigIds: [id]
          }
          delIo(param).then(res => {
            this.listLoading = false
            if(res.code == '200'){
